...
# Files that must never be tracked at all. config.local.json stays forbidden here
# (the shareable repo must not carry anyone's filters). snapshot.json /
# rentcast_usage.json are intentionally committed by the GitHub Actions workflow
# for state persistence (listing data + a counter, no secrets), so not forbidden.
